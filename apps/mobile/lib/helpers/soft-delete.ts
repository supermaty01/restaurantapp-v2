 ...
 */
export async function canDeleteRestaurantPermanently(
  db: DrizzleDatabase,
  restaurantId: number,
): Promise<boolean> {
  ...
  return (dishesCount[0]?.count ?? 0) === 0 && (visitsCount[0]?.count ?? 0) === 0;
}
 ...
 * @returns true si puede ser eliminado permanentemente, false si debe ser soft delete
 */
export async function canDeleteDishPermanently(
  db: DrizzleDatabase,
  dishId: number,
): Promise<boolean> {
  ...
  return (dishVisitsCount[0]?.count ?? 0) === 0;
}
 ...
 */
export async function canDeleteVisitPermanently(
  _db: DrizzleDatabase,
  _visitId: number,
): Promise<boolean> {
...
export async function softDeleteDish(db: DrizzleDatabase, dishId: number): Promise<void> {
  await db.update(schema.dishes).set({ deleted: true }).where(eq(schema.dishes.id, dishId));
}
...
export async function softDeleteVisit(db: DrizzleDatabase, visitId: number): Promise<void> {
  await db.update(schema.visits).set({ deleted: true }).where(eq(schema.visits.id, visitId));
}
