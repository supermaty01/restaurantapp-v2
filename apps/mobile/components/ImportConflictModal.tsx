  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View className="flex-1 justify-center items-center bg-black/50 px-4">
        <View className={`w-full max-w-sm rounded-2xl p-5 bg-surface`}>
          {/* Header */}
          <View className="flex-row items-center justify-between mb-4">
            <View className="flex-row items-center">
              <Ionicons name="alert-circle-outline" size={24} color={colors.accent} />
              <Text className={`text-lg font-bold ml-2 text-ink`}>
          ...
          <ScrollView className="max-h-80">
            <Text className={`text-base mb-4 text-ink-muted`}>
              Ya tienes un restaurante llamado &quot;{conflict.existingEntity?.name}&quot; en tu
              lista.
            </Text>

            <Text className={`text-base mb-2 text-ink-muted`}>
            ...
            <Text className={`text-base font-semibold mb-3 text-ink`}>¿Qué deseas hacer?</Text>
          </ScrollView>
          ...
          <View className="mt-4 gap-3">
            {/* Use existing */}
            <TouchableOpacity
              className={`flex-row items-center p-4 rounded-xl border bg-sunken border-line`}
              onPress={() =>
                conflict.existingEntity &&
                onResolve({ type: 'use_existing', existingId: conflict.existingEntity.id })
              }
            >
              <Ionicons name="link-outline" size={24} color={colors.sage} />
              <View className="ml-3 flex-1">
                <Text className={`font-bold text-ink`}>Usar el restaurante existente</Text>
            ...
            >
              ...
              <View className="ml-3 flex-1">
                <Text className={`font-bold text-ink`}>Crear nuevo restaurante</Text>
                <Text className={`text-sm text-ink-muted`}>
            ...
            {/* Cancel */}
            <TouchableOpacity className={`p-3 rounded-xl bg-sunken`} onPress={onClose}>
              <Text className={`text-center font-semibold text-ink-muted`}>Cancelar</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
